...
df2=df2 %>% mutate(left_censored= CONC_DAILY_UG_M3==0,
...
dplyr::group_by(AQS_PARAMETER_NAME) %>%
  dplyr::summarise(Distinct_Locations = n_distinct(paste(lat, lon)),
        ...
        AboveMDL = 100*(sum(!below_mdl & !left_censored))/length(CONC_DAILY_UG_M3),
...
df_long <- df2 %>%
  pivot_longer(cols = c(MDL_DAILY_STD_UG_M3,CONC_DAILY_UG_M3),
               names_to = "Type",
               values_to = "Value") %>%
  mutate(Type = factor(Type, levels = c("MDL_DAILY_STD_UG_M3", "CONC_DAILY_UG_M3"),
