...
df=as.data.frame(tar_read(covariates_nc))

# remove rows with measurement=0 (will change this later)
df= df %>% filter(CONC_DAILY_STD < 100 )
...
df2=df2 %>% mutate(left_censored= CONC_DAILY_STD==0,
                    below_mdl=CONC_DAILY_STD!=0 & CONC_DAILY_STD<MDL_DAILY_STD_UG_M3)
...
df_stats <- df2 %>%
dplyr::group_by(AQS_PARAMETER_NAME) %>%
  dplyr::summarise(Distinct_Locations = n_distinct(paste(lat, lon)),
        NonDetects = 100*sum(left_censored)/length(CONC_DAILY_STD),
        BelowMDL = 100*sum(below_mdl)/length(CONC_DAILY_STD),
        AboveMDL = 100*(sum(!below_mdl & !left_censored))/length(CONC_DAILY_STD),
...
df_long <- df2 %>%
  pivot_longer(cols = c(MDL_DAILY_STD_UG_M3,CONC_DAILY_STD),
               names_to = "Type",
               values_to = "Value") %>%
  mutate(Type = factor(Type, levels = c("MDL_DAILY_STD_UG_M3", "CONC_DAILY_STD"),
                       labels = c("MDL", "Concentration")))

ggplot(df_long, aes(x = Value, y = AQS_PARAMETER_NAME, fill = Type),rel_min_height = 0.001) +
  geom_density_ridges(alpha = 0.7, scale = 1) + # Alpha for transparency, scale for overlap
  labs(title = "Ridgeline Plot of Chemical Densities",
       x = "Value",
       y = "Chemical",
       fill = "Type") +
...
ggplot() +
  ...
  geom_sf(data = us_map, fill = "lightgray", color = "white") +
  # Plot the unique site locations for the specific chemical
  geom_point(data = chemical_data[[2]], aes(x = lon, y = lat), color = cols[2], alpha = 0.5) +
...
ggplot() +
  ...
  geom_sf(data = us_map, fill = "lightgray", color = "white") +
  # Plot the unique site locations for the specific chemical
  geom_point(data = chemical_data[[3]], aes(x = lon, y = lat), color = cols[3], alpha = 0.5) +
...
ggplot() +
  ...
  geom_sf(data = us_map, fill = "lightgray", color = "white") +
  # Plot the unique site locations for the specific chemical
  geom_point(data = chemical_data[[4]], aes(x = lon, y = lat), color = cols[4], alpha = 0.5) +
