...
################################################################################
AMA_duration2daily_mod <- function(results.dir,amayr,allyears){
 ...
 censmean <- function(conc,mdl,params=NULL, lognormal=F){
  if(is.null(params)){ # If starting values not provided, use data raw mean and sd 
  ...
  if(lognormal==T){ # Adjust for lognormally-distributed data
    mu=exp(mu)
    sigma=exp(sigma)
  }
...
AMA_onesite=AMA_sub %>% subset(AMA_SITE_CODE=="06037CELM" & SAMPLE_DATE== "01/01/2021" & AQS_PARAMETER_CODE==43502)
AMA_min2d_onesite = AMA_onesite %>%
 group_by(SAMPLE_DATE,AQS_PARAMETER_CODE,AMA_SITE_CODE,AQS_POC,QUARTER,YEAR,DURATION_DESC,
      ...
      group_by(SAMPLE_DATE,AQS_PARAMETER_CODE,AMA_SITE_CODE,AQS_POC,QUARTER,YEAR,DURATION_DESC,
               PROGRAM,STATE_ABBR,HOUR,SAMPLING_FREQUENCY_MODE,AQS_PARAMETER_NAME) %>%           
