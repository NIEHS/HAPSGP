...
AMA_file <- function(data.dir,results.dir,amayr,allyears){
  ...
  readme = read.xlsx(paste0(data.dir,'AMA',amayr,'_lookups.xlsx'), sheet = 'readme') # readme
  AMA_analysis = read.xlsx(paste0(data.dir,'AMA',amayr,'_lookups.xlsx'), sheet = 'AMA_analysis') # code
  AMA_LC2STDRatio = read.xlsx(paste0(data.dir,'AMA',amayr,'_lookups.xlsx'), sheet = 'AMA_LC2STDRatio') # code
  AMA_preprocessing = read.xlsx(paste0(data.dir,'AMA',amayr,'_lookups.xlsx'), sheet = 'AMA_preprocessing') # code
  AMA_duration2daily = read.xlsx(paste0(data.dir,'AMA',amayr,'_lookups.xlsx'), sheet = 'AMA_duration2daily') # code
  AMA_daily2annual = read.xlsx(paste0(data.dir,'AMA',amayr,'_lookups.xlsx'), sheet = 'AMA_daily2annual') # code
  AMA_file = read.xlsx(paste0(data.dir,'AMA',amayr,'_lookups.xlsx'), sheet = 'AMA_file') # code
  Durations = read.xlsx(paste0(data.dir,'AMA',amayr,'_lookups.xlsx'), sheet = 'Durations') # sampling durations and minimum sampling counts
  Acronyms = read.xlsx(paste0(data.dir,'AMA',amayr,'_lookups.xlsx'), sheet = 'Acronyms') # information about all acronyms used
  Descriptions = read.xlsx(paste0(data.dir,'AMA',amayr,'_lookups.xlsx'), sheet = 'Descriptions') # final field descriptions

  # read in information about NATTS and format columns
  NATTS = data.table(read.xlsx(paste0('input/AMA',amayr,'_lookups.xlsx'), sheet = 'NATTS')) %>%
  ...
  AQS_CAS = data.table(read.xlsx(paste0(data.dir,'AMA',amayr,'_lookups.xlsx'), sheet = 'AQS_CAS')) %>%
  ...
  NEI = data.table(read.xlsx(paste0(data.dir,'AMA',amayr,'_lookups.xlsx'), sheet = 'NEI')) %>%
  ...
  AirToxScreen = data.table(read.xlsx(paste0(data.dir,'AMA',amayr,'_lookups.xlsx'), sheet = 'AirToxScreen')) %>%
  ...
  write.xlsx(list(readme = readme, Annual_Statistics = annual_ALL, Descriptions = Descriptions,
                  ...
                  AMA_file = AMA_file),
             paste0(data.dir,'AMA',amayr,'_annual.xlsx'))
