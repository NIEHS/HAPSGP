################################################################################
## Modified from original EPA function
## PURPOSE: AMA_duration2daily_mod.R calculates daily averages and statistics per
...
################################################################################
AMA_duration2daily_mod <- function(filelist, data.dir, amayr) {
  ##### PART 1: INITIALIZE GLOBAL VARIABLES #####
  ...
  } # mode string in vector

  # Addition: MLE function for estimating censored data
  cMLE_log <- function(params, y.obs, y.cens = NULL) {
    mu = exp(params[1])
    sigma = exp(params[2])
  ...
  censmean <- function(conc, mdl, params = NULL, lognormal = F) {
    if (is.null(params)) {
      # If starting values not provided, use data raw mean and sd
      mu = mean(conc, na.rm = T)
      sigma = sd(conc, na.rm = T)
    } else {
      mu = params[1]
      sigma = params[2]
    }
    if (lognormal == T) {
      # Adjust for lognormally-distributed data
      mu = exp(mu)
      sigma = exp(sigma)
    }
    obs_data = conc[conc > mdl]
    cens_data = conc[conc < mdl]
    mean_mdl = mean(mdl, na.rm = T)
    cens_data[cens_data == 0] = mean_mdl

    mle <- optim(par = c(mu, sigma), cMLE, y.obs = obs_data, y.cens = cens_data)
  ...
  duration_scales = read.xlsx(
    paste0(data.dir, 'AMA', amayr, '_lookups.xlsx'),
    sheet = 'Durations'
  ) # warning: excel file must be closed
  durcount_hr2d = duration_scales[
    duration_scales$DURATION_DESC == '1 HOUR',
    'MINIMUM_COUNT'
  ] # min # hrs/day to calc dly avg
  dur_minute = duration_scales[
    duration_scales$AVERAGE_TO == 'HOURLY',
    'DURATION_DESC'
  ]
  dur_hour = duration_scales[
    duration_scales$AVERAGE_TO == 'DAILY',
    'DURATION_DESC'
  ]
  ...
  for (i in 1:length(filelist)) {
    ...
    group_by(
      ...
      STATE_ABBR,
      HOUR,
      SAMPLING_FREQUENCY_MODE,
      AQS_PARAMETER_NAME
    ) %>% # group by pollutant/site/poc/day/hour/duration
      summarize(
        N_HR = length(na.omit(CONC_RATIO)), # number of non-null values
        N0_HR = sum(CONC_RATIO == 0, na.rm = T)
      ) %>% # number of zeros
      ungroup() %>%
      ...
      subset(N_HR > N0_HR)
    unique(AMA_min2d$N0_HR)

    AMA_min2d = AMA_min2d %>% subset(N_HR > N0_HR) #%>% #
    max(AMA_min2d$SAMPLE_VALUE_STD, na.rm = T)

    AMA_onesite = AMA_sub %>%
      subset(
        AMA_SITE_CODE == "06037CELM" &
          SAMPLE_DATE == "01/01/2021" &
          AQS_PARAMETER_CODE == 43502
      )
    AMA_min2d_onesite = AMA_onesite %>%
      group_by(
        SAMPLE_DATE,
        AQS_PARAMETER_CODE,
        AMA_SITE_CODE,
      ...
      mutate(mean_cens = censmean(SAMPLE_VALUE_STD, MDL_STD_UG_M3)) %>%
      ungroup() #%>%
    ...
    group_by(
      ...
      summarize(
        CONC_RATIO_HR = mean(CONC_RATIO, na.rm = T), # mean of derived concentrations using LCSTDratio
      ...
      group_by(
        ...
        SAMPLING_FREQUENCY_MODE,
        AQS_PARAMETER_NAME
      ) %>% # group by pollutant/site/poc/day/duration
      summarize(
        CONC_RATIO = mean(CONC_RATIO_HR, na.rm = T), # mean of hourly concentrations
        ...
        N0_DAY = sum(CONC_RATIO_HR == 0, na.rm = T)
      ) %>% # number of hourly zeros values in a day
      ungroup() %>%
      subset(N_DAY >= durcount_hr2d) # subset to days that meet the min number of samples/day
    if (dim(AMA_min2d)[1] > 0) {
      AMA_min2d = AMA_min2d %>%
        mutate_all(~ case_when(is.nan(.) ~ NA, .default = .)) # add catch to convert NaN to NA
    }
    #  sort(unique(AMA_min2d$N0_DAY))
    #Test, is this a daily dataset now?
    #daily_onesite=daily %>% subset(AMA_SITE_CODE == 060371103)
    ...
    AMA_hr2d = AMA_sub %>%
      ...
      mutate(
        ...
        MAX_PULLDATE = max(PULL_DATE, na.rm = T)
      ) %>% # if collocation the max pull date of values
      ungroup() %>%
      subset(
        !(DURATION_DESC == '24 HOURS' &
          ...
          N0_DAY == 1 &
          CONC_RATIO == 0)
      ) %>% # 2 coll vals AND 1 val=0 -> remove the 0
      subset(
        !(DURATION_DESC == '24 HOURS' &
          N_DAY == 2 &
          N0_DAY == 0 &
          ALL_EQUAL == FALSE &
          MIN_PULLDATE != MAX_PULLDATE &
          PULL_DATE == MIN_PULLDATE)
      ) %>% # 2 coll AND non-zero AND diff vals AND diff pull dates -> remove earlier val
      select(
        -c('N_DAY', 'N0_DAY', 'ALL_EQUAL', 'MIN_PULLDATE', 'MAX_PULLDATE')
      ) %>% # remove all fields created for this catch
      ### CATCH END
      group_by(
        AQS_PARAMETER_CODE,
        SAMPLE_DATE,
      ...
      mutate(
        N_DAY = length(na.omit(CONC_RATIO)), # number of non-null values
        N0_DAY = sum(CONC_RATIO == 0, na.rm = T)
      ) %>% # group by pollutant/site/poc/day/duration
      join(duration_scales[, c('DURATION_DESC', 'MINIMUM_COUNT')]) %>% # join min threshold counts
      ...
      summarize(
        ...
        MDL_STD_UG_M3_DAY = mean(MDL_STD_UG_M3, na.rm = T)
      ) #%>% , # mean of hourly MDL in standard conditions
    ungroup() %>%
      join(duration_scales[, c('DURATION_DESC', 'MINIMUM_COUNT')]) #%>% # join min threshold counts
    subset(N_DAY >= MINIMUM_COUNT) %>% # subset to days that meet the min number of samples/day
      setnames(
        old = c('CONC_RATIO_DAY', 'SAMPLE_VALUE_STD_DAY', 'MDL_STD_UG_M3_DAY'),
        new = c('CONC_RATIO', 'SAMPLE_VALUE_STD', 'MDL_STD_UG_M3')
      ) %>% # rename fields so rbind can be used next
    ...
    catch_collocation = AMA_hr2d %>%
      subset(DURATION_DESC == '24 HOURS' & N_DAY > 1)
    if (dim(catch_collocation)[1] != 0) {
      print("Collocation by pollutant/site/POC/day/'24 HOURS'. Stop and fix.")
    }
    ...
    # If more than 50% of POCs record 0 then daily value will be 0
    # If 50% or less 0s, value will be mean of non-0 measurements
    daily = rbind(AMA_min2d, AMA_hr2d) %>%
      group_by(
        ...
        DURATION_DESC,
        AQS_PARAMETER_NAME
      ) %>% # group by pollutant/site/day/duration (across pocs)
      mutate(
        CONC_DAILY = mean(CONC_RATIO, na.rm = T), # mean of values
        N_DAILY = length(na.omit(CONC_RATIO)), # number of non-null values
        N0_DAILY = sum(CONC_RATIO == 0, na.rm = T)
      ) %>% # number of zeros
      ...
      subset(
        !(N_DAILY > 1 &
          CONC_DAILY != 0 &
          FRAC_ZERO > frac_remove &
          CONC_RATIO != 0)
      ) %>% # multiple samps/day AND mean(samps)!=0 AND high % of 0s -> remove non-zeros
      subset(
        !(N_DAILY > 1 &
          CONC_DAILY != 0 &
          FRAC_ZERO <= frac_remove &
          CONC_RATIO == 0)
      ) %>% # multiple samps/day AND mean(samps)!=0 AND low % of 0s -> remove zeros
    ...
    group_by(
      ...
      summarize(
        CONC_DAILY_UG_M3 = mean(CONC_RATIO, na.rm = T), # mean of values
        ...
        ZERO_COUNT = length(na.omit(CONC_RATIO) == 0)
      ) %>% # number of zeros
      mutate(across(
        c(CONC_DAILY_UG_M3, CONC_DAILY_STD, MDL_DAILY_STD_UG_M3),
        ~ ifelse(is.nan(.), NA, .)
      )) %>% # add catch to convert NaN to NA
      ungroup()
    ##### PART 5: CALCULATE DAILY AVERAGES FOR REMOTE DATA #####
    ...
    daily_REMOTE = AMA %>%
      ...
      group_by(
        ...
        DURATION_DESC,
        AQS_PARAMETER_NAME
      ) %>% # group by pollutant/site/day/duration
      summarize(
        CONC_DAILY_UG_M3 = mean(CONC_RATIO, na.rm = TRUE), # mean of values
        ...
        SAMPLING_FREQUENCY_DAILY = mode_STR(SAMPLING_FREQUENCY_MODE)
    ...
    save(
      ...
      daily_REMOTE,
      AMA_POCCOUNT_REMOTE,
      file = paste0(results.dir, 'AMA', amayr, '_daily_', allyears[i], '.Rda')
    )
