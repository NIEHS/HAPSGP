...
# Wind speed and direction calculation from components
ucol = "NARR_0_UWND.10M_00000"
vcol = "NARR_0_VWND.10M_00000"
covs_tx$ws = sqrt(covs_tx[[ucol]]^2 + covs_tx[[vcol]]^2)
covs_tx$wd = (atan2(-covs_tx[[ucol]], -covs_tx[[vcol]]) * 180 / pi) %% 360
...
wf = windfreq(
  ...
  ws_col = "ws",
  wd_col = "wd",
  ws.int = max(covs_tx$ws), # change to 1
  wd.int = 4,
  calm.thres = 0,
  statistic = "fraction",
  format = "data.frame",
...
plot(
  unlist(as.vector(res_sedc_return[, 2:13])),
